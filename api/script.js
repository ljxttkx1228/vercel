module.exports = (req, res) => {
  const luaScript = `local Players = game:GetService("Players")
...
if success and result then
    ...
    error("[MoonUILibrary] 加载失败！请检查网络连接或链接是否有效：" .. tostring(result))
end
...
local configFile = "Moon_X_DE_config.json"
local config = {
    webhookUrl = "",
    notifyCash = false,
...
local toggleCurrency = UILibrary:CreateToggle(currencyNotifyCard, {
    ...
    Callback = function(state)
        ...
        UILibrary:Notify({ Title = "配置更新", Text = "金额变化监测: " .. (state and "开启"或"关闭"), Duration = 5 })
        saveConfig()
    end
})
...
local toggleLeaderboard = UILibrary:CreateToggle(leaderboardNotifyCard, {
    ...
    Callback = function(state)
        ...
        UILibrary:Notify({ Title = "配置更新", Text = "排行榜状态监测: " .. (state and "开启"或"关闭"), Duration = 5 })
        saveConfig()
        return nil
    end
})
...
local toggleLeaderboardKick = UILibrary:CreateToggle(leaderboardKickCard, {
    ...
    Callback = function(state)
        ...
        UILibrary:Notify({ Title = "配置更新", Text = "上榜自动踢出: " .. (state and "开启"或"关闭"), Duration = 5 })
        saveConfig()
        return nil
    end
})
